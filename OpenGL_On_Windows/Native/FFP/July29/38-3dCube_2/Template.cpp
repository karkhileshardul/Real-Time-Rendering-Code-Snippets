...
HDC ghdc;
HGLRC ghrc;
GLfloat angleCube_degree = 0.0f;
GLfloat angleCube_radian = 0.0f;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	HWND hwnd;
	MSG msg;
	TCHAR szClassName[] = TEXT("38 - 3dCube_2");
	int nxIndex, nyIndex;
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW, szClassName, TEXT("36 - 3dCube_2 Window"), WS_OVERLAPPED | WS_CAPTION | WS_SYSMENU | WS_THICKFRAME | WS_MAXIMIZEBOX | WS_MINIMIZEBOX | WS_CLIPSIBLINGS | WS_CLIPCHILDREN, (nxIndex / 2 - (WIN_WIDTH / 2)), (nyIndex / 2 - (WIN_HEIGHT / 2)), WIN_WIDTH, WIN_HEIGHT, NULL, NULL, hInstance, NULL);
	ghwnd = hwnd;
	if (hwnd == NULL) {
...
void display(void) {
	...
	angleCube_radian = angleCube_degree*(M_PI / 180.0f);
	...
	rotation_x_Matrix[5] = cos(angleCube_radian);
	rotation_x_Matrix[6] = sin(angleCube_radian);
	rotation_x_Matrix[7] = 0.0f;
	rotation_x_Matrix[8] = 0.0f;
	rotation_x_Matrix[9] = -sin(angleCube_radian);
	rotation_x_Matrix[10] = cos(angleCube_radian);
	...
	rotation_z_Matrix[0] = cos(angleCube_radian);
	rotation_z_Matrix[1] = sin(angleCube_radian);
	rotation_z_Matrix[2] = 0.0f;
	rotation_z_Matrix[3] = 0.0f;
	rotation_z_Matrix[4] = -sin(angleCube_radian);
	rotation_z_Matrix[5] = cos(angleCube_radian);
...
void update(void) {
	...
	angleCube_degree = angleCube_degree + 0.1f;

	if (angleCube_degree >= 360.0f) {
		
		angleCube_degree = 0.0f;
	}
}
