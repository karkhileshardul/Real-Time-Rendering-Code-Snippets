...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	HWND hwnd;
	MSG msg;
	TCHAR szClassName[] = TEXT("20-ConcentricCircles");
	bool bDone = false;
	...
	hwnd = CreateWindowEx(WS_EX_APPWINDOW,
		szClassName,
		TEXT("20-ConcentricCircles Window"),
...
void DrawCircle()
{
	...
	distance_A = sqrt((-1.0f - 0.0f)*(-1.0f - 0.0f) + (-1.0f - 1.0f)*(-1.0f - 1.0f));
	distance_B = sqrt((1.0f - (-1.0f))*(1.0f - (-1.0f)) + ((-1.0f) - (-1.0f))*((-1.0f) - (-1.0f)));
	distance_C = sqrt((0.0f - 1.0f)*(0.0f - 1.0f) + (1.0f - (-1.0f))*(1.0f - (-1.0f)));
	incenter_x = (distance_A * (-1.0f)) + (distance_B * (1.0f)) + (distance_C * (0.0f)) / (distance_A + distance_B + distance_C);
	incenter_y = (distance_A * (-1.0f)) + (distance_B * (-1.0f)) + (distance_C * (1.0f)) / (distance_A + distance_B + distance_C);

	semi_perimeter = (distance_A + distance_B + distance_C) / 2;

	radius = sqrt(semi_perimeter*(semi_perimeter - distance_A) * semi_perimeter*(semi_perimeter - distance_B) * semi_perimeter*(semi_perimeter - distance_C)) / semi_perimeter;
	glLineWidth(3.0f);
