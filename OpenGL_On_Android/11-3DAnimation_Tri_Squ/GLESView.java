package com.astromedicomp.win_3danimation_tri_squ;
...
public class GLESView extends GLSurfaceView implements GLSurfaceView.Renderer, OnGestureListener,OnDoubleTapListener{
	...
	private float pyramid_angle=0.0f;
	...
	private void initialize(GL10 gl){
		...
		GLES30.glGenBuffers(1,vbo_pyramid_color ,0);
		GLES30.glBindBuffer(GLES30.GL_ARRAY_BUFFER,vbo_pyramid_color[0]);
		...
		GLES30.glBufferData(GLES30.GL_ARRAY_BUFFER,pyramidColors.length*4,
							verticesBuffer,GLES30.GL_STATIC_DRAW);
		...
		GLES30.glGenBuffers(1,vbo_cube_color ,0);
		GLES30.glBindBuffer(GLES30.GL_ARRAY_BUFFER,vbo_cube_color[0]);
		...
		GLES30.glBufferData(GLES30.GL_ARRAY_BUFFER,cubeColors.length*4,
							verticesBuffer,GLES30.GL_STATIC_DRAW);
	...
	public void display(){
		...
		Matrix.rotateM(rotationMatrix,0,pyramid_angle,0.0f,1.0f,0.0f);
		...
		Matrix.setIdentityM(modelViewProjectionMatrix,0);
		Matrix.translateM(modelViewMatrix,0,1.5f,0.0f,-6.0f);
		Matrix.rotateM(rotationMatrix,0,cube_angle,cube_angle,cube_angle,cube_angle);
		Matrix.multiplyMM(resultMatrix,0,
							modelViewMatrix,0,
							rotationMatrix,0);
	...
	void uninitialize(){
		...
		if(vbo_pyramid_color[0]!=0){
			GLES30.glDeleteBuffers(1,vbo_pyramid_color,0);
	...
	private void update(){
	pyramid_angle=pyramid_angle+2.0f;
	if(pyramid_angle>=360.0f){
		pyramid_angle=pyramid_angle-360.0f;
	}

	cube_angle=cube_angle+2.0f;
