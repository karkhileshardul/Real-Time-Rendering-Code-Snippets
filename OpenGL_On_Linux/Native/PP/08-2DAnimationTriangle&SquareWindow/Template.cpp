...
GLfloat tri_angle=0.0f;
GLfloat squ_angle=0.0f;
...
void CreateWindow(void)
{
	...
	XStoreName(gpDisplay,gWindow,"08-2D Animation Triangle & Square Window");
...
void initialize(void)
{
	...
	glVertexAttrib3f(SSK_ATTRIBUTE_COLOR,0.61f,0.79f,0.66f);
	glBindBuffer(GL_ARRAY_BUFFER,0);
...
void display(void)
{
	...
	rotationMatrix=vmath::rotate(tri_angle,0.0f,1.0f,0.0f);
	...
	modelViewMatrix=vmath::translate(1.5f,0.0f,-6.0f);
	rotationMatrix=vmath::rotate(squ_angle,1.0f,0.0f,0.0f);
	modelViewProjectionMatrix=gPerspectiveProjectionMatrix*modelViewMatrix*rotationMatrix;
	glUniformMatrix4fv(gMVPUniform,1,GL_FALSE,modelViewProjectionMatrix);
	glBindVertexArray(gVao_square);
...
void update(void){
	tri_angle=tri_angle+0.8f;
	if(tri_angle>=360.0f){
		tri_angle=tri_angle-360.0f;
	}

	squ_angle=squ_angle+0.8f;
	if(squ_angle>=360.0f){
