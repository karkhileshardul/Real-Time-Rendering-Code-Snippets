...
ID3D11Buffer *gpID3D11Buffer_VertexBufferColor=NULL;		
...
int WINAPI WinMain(HINSTANCE hInstance,HINSTANCE hPrevInstance,LPSTR lpszCmdLine,int iCmdShow){
	...
	HWND hwnd;
	MSG msg;
	TCHAR szClassName[]=TEXT("04-Direct3D11 Multicolour Triangle Window");
	bool bDone=false;
...
/*
	const char *vertexShaderSourceCode=
		...
		"}";
*/
	//8th Line in vertexShaderSourceCode ==>input layout la dilelya color cha and ithlya COLOR cha sambandha nahi
	//10th Line in vertexShaderSourceCode ==>main() cha parameter ha input layout la dilela position and input layout la dilela color aahe
...
/*	const char *pixelShaderSourceCode=
		...
		"{"\
			"float4 output_color=col;"\
			"return(output_color);"\
		"}";
*/
	//pixel shader decides what is coming 'in' on basis of who are the parameter (it doesn't matters whether the color comes from struct or naked)
	...
	hr=gpID3D11Device->CreateBuffer(&bufferDesc,NULL,&gpID3D11Buffer_VertexBufferPosition);
	if(FAILED(hr)){
		fopen_s(&gpFile,gszLogFileName,"a+");
	...
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBufferPosition,NULL,
...
/*		3 ___START___		*/
	...
	bufferDesc.BindFlags=D3D11_BIND_VERTEX_BUFFER;
	bufferDesc.CPUAccessFlags=D3D11_CPU_ACCESS_WRITE;
	hr=gpID3D11Device->CreateBuffer(&bufferDesc,NULL,&gpID3D11Buffer_VertexBufferColor);
	if(FAILED(hr)){
		fopen_s(&gpFile,gszLogFileName,"a+");
		fprintf_s(gpFile,"ID3D11Device::CreateBuffer() Failed for Vertex Buffer Position..\n");
		fclose(gpFile);
		gpFile=NULL;
		return (hr);
	}else{
		fopen_s(&gpFile,gszLogFileName,"a+");
		fprintf_s(gpFile,"ID3D11Device::CreateBuffer() Succeded for Vertex Buffer Position..\n");
		fclose(gpFile);
	}

	ZeroMemory(&mappedSubresource,sizeof(D3D11_MAPPED_SUBRESOURCE));
	gpID3D11DeviceContext->Map(gpID3D11Buffer_VertexBufferColor,NULL,
								D3D11_MAP_WRITE_DISCARD,NULL,
								&mappedSubresource);	
	memcpy(mappedSubresource.pData,colors,sizeof(colors));
	gpID3D11DeviceContext->Unmap(gpID3D11Buffer_VertexBufferColor,NULL);
...
void display(void){
	...
	UINT offset=0;
	gpID3D11DeviceContext->IASetVertexBuffers(0,1,&gpID3D11Buffer_VertexBufferPosition,&stride,&offset);
	//2nd Parameter is Input slot
	
	stride=sizeof(float)*3;			//r,g,b so 3 ni Multiply kelai
	offset=0;
	gpID3D11DeviceContext->IASetVertexBuffers(1,1,&gpID3D11Buffer_VertexBufferColor,&stride,&offset);
	//2nd Parameter is Input slot
	...
	viewMatrix=XMMatrixTranslation(0.0f,0.0f,6.0f);
...
void uninitialize(void){
	...
	if(gpID3D11Buffer_VertexBufferPosition){
		gpID3D11Buffer_VertexBufferPosition->Release();
		gpID3D11Buffer_VertexBufferPosition=NULL;
	}

	if(gpID3D11Buffer_VertexBufferColor){
		gpID3D11Buffer_VertexBufferColor->Release();
		gpID3D11Buffer_VertexBufferColor=NULL;
	}
