...
int WINAPI WinMain(HINSTANCE hInstance,HINSTANCE hPrevInstance,LPSTR lpszCmdLine,int iCmdShow){
	...
	HWND hwnd;
	MSG msg;
	TCHAR szClassName[]=TEXT("03-Direct3D11 Perpsective Triangle Window");
	bool bDone=false;
...
HRESULT resize(int width,int height){
	...
	if(width<=height){
		gPerspectiveProjectionMatrix=XMMatrixPerspectiveFovLH(45.0f,(float)height/(float)width,
																0.1f,100.0f);
	}else{
		gPerspectiveProjectionMatrix=XMMatrixPerspectiveFovLH(45.0f,(float)width/(float)height,
																0.1f,100.0f);
	}
	return (hr);
}
