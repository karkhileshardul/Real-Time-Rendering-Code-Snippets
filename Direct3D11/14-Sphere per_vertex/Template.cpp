...
int WINAPI WinMain(HINSTANCE hInstance,HINSTANCE hPrevInstance,LPSTR lpszCmdLine,int iCmdShow){
	...
	HWND hwnd;
	MSG msg;
	TCHAR szClassName[]=TEXT("15-Direct3D11 Sphere per_vertex Window");
...
//	dxgiSwapChainDesc.SampleDesc.Quality=1;
	...
	const char *vertexShaderSourceCode=
		...
		"{"\
			...
			"{"\
				//float4 eye_coordinates=mul(u_view_matrix,mul(u_world_matrix,pos));
				...
				"float3 light_direction=normalize(((float3)(u_light_position) - eye_coordinates.xyz));"\
				...
				"float4 specular=u_Ls * u_Ks * pow(max(dot(reflection_vector,viewer_vector),0.0),u_material_shininess);"\
				"output.phong_ads_color=(float4)ambient + diffuse + specular;"\
			"}"\
			"else"\
			"{"\
...
void display(void){
	...
	if(gbLight==true){
		...
		constantBuffer.material_shininess_uniform=material_shininess;//XMVectorSet(material_shininess,0.0f,0.0f,0.0f);
	}else{
