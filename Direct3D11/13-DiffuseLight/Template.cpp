...
struct  CBUFFER{
	...
	XMVECTOR gLd;
	XMVECTOR gKd;
	XMVECTOR gbLightPosition;
	unsigned gLKeyPressed;
};
...
int WINAPI WinMain(HINSTANCE hInstance,HINSTANCE hPrevInstance,LPSTR lpszCmdLine,int iCmdShow){
	...
	HWND hwnd;
	MSG msg;
	TCHAR szClassName[]=TEXT("12-Direct3D11 Diffuse Light Window");
...
//XMVECTOR la float4
	...
	const char *vertexShaderSourceCode=
		...
		"{"\
			...
			"{"\
				...
				"output.diffuse_light=(float4)(mul((u_Ld*u_Kd) ,(max(dot(s,tnorm),0.0))));"\
			"}"\
			"else"\
			"{"\
	...
	"float4 main(float4 pos:SV_POSITION,float4 diffuse_color:COLOR):SV_TARGET"\
	"{"\
		"float4 color;"\
		"color=diffuse_color;"\
		"return color;"\
...
void display(void){
	...
	if(gbLight==true){
		constantBuffer.gLKeyPressed=1;		
		constantBuffer.gLd=XMVectorSet(1.0f, 1.0f, 1.0f, 0.0f);
		constantBuffer.gKd=XMVectorSet(0.5f, 0.5f, 0.5f, 0.0f);
		constantBuffer.gbLightPosition=XMVectorSet( 0.0f, 0.0f, -2.0f, 1.0f);
	}else{
		constantBuffer.gLKeyPressed=0;		
	}
	...
	worldviewMatrix=rotationMatrix*worldMatrix;
